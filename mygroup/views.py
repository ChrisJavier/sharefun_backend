...
from .forms import AddForm, ListForm
...
def add(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            form.save()
    return render(request, 'mypost/add.html', {'form': form, 'post_category': post_category})
...
def all_posts(request):
    ...
    return render(request, 'mypost/all_posts.html', context_posts)


def show_post(request, pk):
    post = get_object_or_404(Post, pk=pk)
    brand = Category.objects.get(id=post.category)
    return render(request, 'mypost/detail_post.html', {'post': post, 'brand': brand})
